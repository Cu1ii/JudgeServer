...
func judge(id int, code, language, problem string, contest int, username, oj, ojpro string,
	...
	for idx, in := range inputFiles {
		...
		if result.Result != 0 {
			// 据 LOJ 源码中反应, QDU 判 Memory Exceed 有谜之 Bug, 这里还没有确认, 所以先使用 LOJ 处理方式
...
func singleJudge(timeLimit, memoryLimit, id int, inputPath, outputPath, errorPath, logPath, language string) (*judger.Result, error) {
	switch language {
	case "C":
